...
X1ext <- cbind(1, X1)
X2ext <- cbind(1, X2)
#imprime a saida para diferentes valores de neuronios na camada escondida
for (i in seq(1, 3, 1)) {
  num_neuronios <- 5*i
  retlist <- trainELM(X1ext, Y1, num_neuronios, 0)
  W <- retlist[[1]]
  H <- retlist[[2]]
  Z <- retlist[[3]]
  Yhat <- sign(YELM(X2ext, Z, W, 0))
  error <- sum((Y2 - Yhat)^2)/(length(Yhat)*4)
